...
public class JsonInscricaoReader {
	...
	/*
	 * Carrega uma prova escrita apartir da representação Json
	 */
	private void carregaProvasEscritas(JsonObject json, InscricaoEdital inscricaoEdital) {
		...
		for(int i = 0; i < jsonProvasEscritas .size(); i++) {
			...
			carregaNotasQuestoesIniciais(jsonProva.get("notaOriginalQuestao").getAsJsonArray(), inscricaoEdital.pegaAvaliacaoProvaEscrita(i), leitorQuestoes);
			carregaNotasQuestoesRecurso(jsonProva.get("notaRecursoQuestao").getAsJsonArray(), inscricaoEdital.pegaAvaliacaoProvaEscrita(i), leitorQuestoes);
